#!/usr/bin/env python3
"""Descarga TODOS los días de 2025 (SST + clorofila) desde CDS, guarda recortes
...
def parse_date_from_name(name: str, kind: str) -> str | None:
    """Devuelve YYYY-MM-DD a partir del nombre del NetCDF CDS."""
    base = Path(name).name
    if kind == "sst":
        ...
        # ...-20250101-fv6.0.nc or ...-202501-... for monthly (we expect daily)
...
def month_daily_complete(kind: str, year: int, month: int) -> bool:
    days = days_in_month(year, month)
    existing = sum(1 for d in days if (DAILY / f"{kind}_{year}-{month:02d}-{d}.nc").exists())
    # Completo si tenemos casi todos (>= 80% o todos). Para skip de descarga usamos umbral alto.
    return existing >= len(days)


def download_month_daily(client: cdsapi.Client, kind: str, year: int, month: int) -> list[Path]:
    """Descarga todos los días del mes, recorta y guarda. Borra zip/global tras procesar."""
    days = days_in_month(year, month)
    saved: list[Path] = []
    # Reutilizar días ya guardados
...
def monthly_mean_from_daily(kind: str, year: int) -> Path | None:
    """Promedia diarios por mes y guarda stack mensual + diarios ya están conservados."""
    ...
    for month in range(1, 13):
        ...
        # promedio aritmético sobre días disponibles
...
def plot_monthly_grid(stack_path: Path, kind: str) -> None:
    ...
    fig, axes = plt.subplots(
        nrows, ncols, figsize=(12.5, 3.1 * nrows), subplot_kw={"projection": ccrs.PlateCarree()},
    ...
    if mesh is not None:
        cbar = fig.colorbar(mesh, ax=axes.ravel().tolist(), shrink=0.85, pad=0.02)
...
def write_manifest() -> None:
    ...
    lines = [
        f"# Datos 2025 — diarios + promedios mensuales (calculados)",
        "",
        f"Bbox: {LAT_S},{LON_W},{LAT_N},{LON_E}",
        "",
        "## Diarios (conservar para evaluaciones futuras)",
        f"- SST: {len(sst_days)} archivos en `data/daily_2025/sst_YYYY-MM-DD.nc`",
        f"- CHL: {len(chl_days)} archivos en `data/daily_2025/chl_YYYY-MM-DD.nc`",
        "",
        "## Promedios mensuales (media aritmética de días disponibles)",
        f"- `data/monthly_from_daily_2025/sst_YYYY-MM_mean.nc`",
        f"- `data/monthly_from_daily_2025/chl_YYYY-MM_mean.nc`",
